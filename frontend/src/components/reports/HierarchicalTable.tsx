...
function RootNodeRow({ nodo }: { nodo: ReportNode }) {
  ...
  return (
    <div className="flex flex-col border border-white/5 rounded-[2rem] overflow-hidden bg-surface/40 backdrop-blur-sm transition-all duration-500">
      <div 
        onClick={() => setIsExpanded(!isExpanded)}
        className={`p-6 flex items-center justify-between cursor-pointer transition-all duration-300 hover:bg-white/[0.04] relative group ${isIngreso ? 'border-l-4 border-primary' : 'border-l-4 border-error/50'}`}
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <span className={`text-xl font-black tracking-[0.1em] text-text-prime uppercase ${isFtStyle ? 'font-mono' : ''}`}>{nodo.nombre}</span>
            <span className="text-[10px] uppercase tracking-widest text-text-muted/40 font-bold">Categoría Principal</span>
          </div>
        </div>
        
        <div className="flex items-center gap-8">
          <div className="hidden md:flex flex-col items-end opacity-40">
             <span className="text-[10px] uppercase tracking-widest font-black text-text-muted">Estado del Flujo</span>
             <div className="flex items-center gap-1">
                {isIngreso ? <TrendingUp size={12} className="text-success" /> : <TrendingDown size={12} className="text-error" />}
                <span className="text-[10px] font-bold uppercase text-text-muted">{isIngreso ? 'Entrante' : 'Saliente'}</span>
             </div>
          </div>
      ...
      >
        ...
        <div className="flex items-center gap-6">
          ...
          <button
            ...
            className="opacity-0 group-hover/cat:opacity-100 transition-all duration-300 w-8 h-8 rounded-lg bg-primary/10 text-primary hover:bg-primary hover:text-black flex items-center justify-center"
...
function SubCategoryRow({ nodo, isIngreso }: { nodo: ReportNode, isIngreso: boolean }) {
  ...
  return (
    <div className="flex flex-col">
      <div 
        onClick={() => setIsExpanded(!isExpanded)}
        className="px-16 py-3 flex items-center justify-between cursor-pointer hover:text-primary transition-all duration-200 border-l border-white/5 ml-12 group/sub"
      >
        <div className="flex items-center gap-3">
          <div className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${isExpanded ? 'bg-primary scale-125' : 'bg-white/10'}`}></div>
      ...
      {isExpanded && nodo.movimientos.length > 0 && (
        <div className="mx-20 my-4 rounded-2xl border border-white/5 bg-surface/20 overflow-hidden shadow-inner animate-in zoom-in-95 duration-300">
          <table className="w-full text-left text-[10px]">
            <thead className="bg-white/5 text-text-muted/40 uppercase tracking-widest font-black">
              <tr>
