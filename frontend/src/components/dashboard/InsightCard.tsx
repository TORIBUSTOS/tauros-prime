    ...
    <BaseCard className="h-full flex flex-col justify-between group insight-card" hoverable>
      <div className="insight-body">
        <div className="flex items-center justify-between mb-6 insight-header">
          <div className="flex items-center gap-3">
            ...
            <span className={`text-[10px] uppercase font-bold tracking-widest ${accentColorClass} insight-type`}>
              {insight.type}
            </span>
            {count > 1 && (
              <span className="text-[9px] font-black bg-white/10 text-imperial-text-muted/60 px-2 py-0.5 rounded-full border border-white/10 uppercase tracking-widest">
                ×{count}
              </span>
            )}
          </div>
          <button 
            ...
            className="text-[10px] text-imperial-text-muted/40 group-hover:text-imperial-bronze transition-all flex items-center gap-1 uppercase tracking-tighter font-bold analyze-button"
          >
        ...
        <div className="space-y-2">
          <h4 className="text-lg font-bold text-imperial-text-prime tracking-tight category-title">{insight.categoria}</h4>
          <p className="text-sm text-imperial-text-muted/70 leading-relaxed font-medium insight-description">
            {insight.insight}
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 pt-4 border-t border-white/5 flex justify-between items-center insight-footer">
        <div className="flex flex-col">
          <span className="text-[10px] text-imperial-text-muted/30 uppercase tracking-[0.1em] font-bold">Confianza del IA Motor</span>
          <div className="h-1 w-24 bg-white/5 rounded-full mt-1.5 overflow-hidden">
            <div 
              className={`h-full transition-all duration-1000 ease-out shadow-[0_0_8px_currentColor] progress-bar ${isHighConfidence ? 'bg-success' : isLowConfidence ? 'bg-error' : 'bg-imperial-bronze'}`}
              style={{ width: `${insight.confidence * 100}%` }}
            />
          </div>
        </div>
        <span className={`text-sm font-black italic ${accentColorClass} impact-value`}>
